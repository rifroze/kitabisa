...
class SempoaaUITests: XCTestCase {
    ...
    func testDisplayingOutput() {
        ...
        let element2 = app.children(matching: .window).element(boundBy: 0).children(matching: .other).element(boundBy: 1).children(matching: .other).element.children(matching: .other).element
        ...
        XCTAssertEqual(textView.value as! String, "25", "Test output content with first cell")
    }
    ...
    func testSwipeCards() {
        ...
        let element3 = app.children(matching: .window).element(boundBy: 0).children(matching: .other).element(boundBy: 1).children(matching: .other).element.children(matching: .other).element
        ...
        XCTAssertEqual(textView.value as! String, "78", "Test output content with second cell")
    }
